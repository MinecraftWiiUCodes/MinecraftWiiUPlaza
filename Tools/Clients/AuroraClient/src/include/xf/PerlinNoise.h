...
namespace Noise {
    class PerlinNoise {
    public:
        PerlinNoise(double new_seed = 1337) : seed{new_seed} {
            for (int i = 0; i < 256; i++) {
			    p[256 + i] = p[i] = permutation[i];
		    }
        }

        void setSeed(double new_seed) { seed = new_seed; }
        double getSeed()              { return seed;     }

        double noise(double x, double y, double z, double amplitude = 1, double frequency = 1) {
		    double value = 0.0;
		    double size = default_size;
		    double initialSize = size;

		    while (size >= 1) {
			    value += smoothNoise((x / size) * frequency, 
				                     (y / size) * frequency, 
									 (z / size) * frequency) * size;
			    size /= 2.0;
		    }

		    return amplitude * (value / initialSize);
	    }
